...
Web `StatsLine` 通过标准 `useProjection` 席位读取 token 用量，另一个 `sessionStats` 投影则提供全日志的计数、墙钟时间与 TTFT。压缩使可见 assistant 步骤归零后，持久 token 与上下文分组仍会保留。缓存写入会计入计费输入和缓存命中率分母；扩展拥有的可选 `liveTokenUsage` 值可以临时替换界面显示的输入／输出计数项，但不会成为计费权威。未部署 token-meter 时会去掉 token 分组；只有压力与容量都已知时才显示占用率。
