...
客户端词汇表还为可选扩展插件保留了 `liveTokenUsage`。它镜像持久用量计数项，增加 `estimated` 标记与可选的 `tokensPerSecond`，但 token-meter 自身不注册该投影；生产方拥有其估算、校正与更新频率，而 `tokenUsage` 仍是精确持久用量的权威来源（[决策](../../../.agents/notes/implemented/feature/2026-08-04-optional-live-token-usage-projection.md)）。
