...
The client vocabulary additionally reserves `liveTokenUsage` for an optional extension plugin. It mirrors the durable usage buckets, adds an `estimated` marker and optional `tokensPerSecond`, and is not registered by token-meter itself; producers own their estimation, correction, and update cadence, while `tokenUsage` remains the exact durable authority ([decision](../../../.agents/notes/implemented/feature/2026-08-04-optional-live-token-usage-projection.md)).
