...
聊天统计行通过标准套件 `useProjection` 从通用 token-meter 投影 `tokenUsage` 取得精确 token 账目：计费输入为未缓存输入、缓存读取与缓存写入之和；缓存命中率以缓存读取除以该总量。扩展还可以提供 `liveTokenUsage`；该投影存在时，统计行使用其中持续变化的输入与输出计数项，为启发式数值添加 `~` 前缀，并以不缩写、按数位分组的形式显示计费输入与输出之和；实时投影消失或结算后，统计行回退到持久的精确值。轮次与步骤计数、LLM（大语言模型）与工具墙钟时间以及平均 TTFT（首 token 延迟）都来自全日志的 `sessionStats` 投影（Host 端从步边界、首 token chunk、工具配对与已组装消息折算），因此分页与压缩都无法改变这些数字；未组合该单元的装配回退为对可见节点做窗口折算，其字段与投影一一对应。内建统计条刻意不渲染吞吐率：回放墙钟时间与提供方报告的输出用量都不能衡量活跃流的速率；外部 `conversation.composer.dock` 扩展则可从自己的实时投影渲染吞吐率。每个已结算轮次仍会单独在 assistant footer 的 `用时` 之后保留 hover 才显示的 `首 token {s}秒 · {tps} tok/s` 标签——即该轮次首个步骤的 TTFT 与轮次聚合的解码吞吐——仅在窗口内计时完整时显示，未记录的数字会各自省略。未组合 token-meter 的部署会整组省略 token 分组。统计行为累计总量预留额外宽度，保持紧凑间距，并让长内容保持可见，不以省略号裁切。上下文占用率渲染为 composer 尾部的 ContextMeter：模型座位之后的一枚 14px 占用圆环，由 `contextPressure` 供数，仅当分子与路由容量都已知时才渲染；点击弹出的面板把「已用百分比」标题与 `~已用 / 容量` 数字，与来自 `contextBreakdown` 投影、带 `~` 前缀的启发式组成明细行（系统提示词、工具、对话消息）及分色分段进度条并列。圆环与标题读取 `projectedTokens`——把提供方样本沿此后表层的增减推进到当下——因此压缩会立刻反映出来，而不必再等一整轮；组成明细行仍是纯启发式，因此加起来依然不等于标题数字（[原理](../../llm/token-meter/README.md)）。占用率是刻意为之的近似值：分子与容量是两个相互独立的「后写覆盖」投影字段，并非同一次请求的原子观测。
